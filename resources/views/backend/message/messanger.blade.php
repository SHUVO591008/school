				...
				<div class="col-md-4">
					<div class="card">
						...
						<div class="card-body contacts_body">
							<ui class="contacts activeClass">

								@foreach ($user as $key=>$item)

									<a style="text-decoration: none" href="{{URL::to('/messages/'.$item->id)}}">
                                        <li id="row{{ $item->id }}"  style="cursor: pointer" class="{{ ($item->id==$recever)?"active":'' }}">
                                            <div class="d-flex bd-highlight">
                                                <div class="img_cont">
                                                    <img src="{{ URL::asset($item->image) }}" class="rounded-circle user_img">
                                                    <span id="online" class="{{ ($item->online==0)?"":"online_icon" }}"></span>


                                                </div>
                                                <div class="user_info">
                                                    <span>{{ $item->name }}</span>
                                                    <p>{{ $item->name }} {{ ($item->online==0)?"is Ofline":"is online" }} </p>
                                                </div>
                                            </div>
                                        </li>
                                    </a>
